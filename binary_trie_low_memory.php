...
/**
 * SplFixedArrayを使用することで消費メモリを抑えたBinaryTrie
 * その分やや速度は遅くなっている
 */
class Binary_Trie_Low_Memory
{
	/** @var SplFixedArray ポインタ */
	private $nodes;
	/** @var SplFixedArray 部分木に含まれる要素の個数 */
	private $cnt;
	/** @var int 最大のポインタID */
	private $id;
	/** @var int ビット長 */
	...
	{
		$n = $max_query * $bitlen;
		$this->nodes = new SplFixedArray(2 * $n);
		for ($i = 0; $i < 2 * $n; ++$i) {
			$this->nodes[$i] = -1;
		}
		$this->cnt = new SplFixedArray($n);
		for ($i = 0; $i < $n; ++$i) {
			$this->cnt[$i] = 0;
		}
		$this->id = 0;
